...
<div class="row">
	<?php if ($logo) { ?>
		<div id="logo" class="col-xs-12 col-sm-12 col-md-4 col-lg-3">
			...
			<?php } else { ?>
				<a href="<?php echo $home; ?>"><img src="<?php echo $logo; ?>" title=" " alt=" " /></a>
			<?php } ?>
		</div>
	<?php } ?>
	<div class="col-xs-12 col-sm-12 col-md-8 col-lg-9">
		...
		<div id="links" class="col-xs-12 col-sm-12 col-md-8 col-lg-9 col-md-pull-4 col-lg-pull-3">
			...
			<ul>
				<?php if (isset($headerlinks)) { ?>
					<?php foreach ($headerlinks as $headerlink) { ?>
						<li><a href="<?php echo $headerlink['link']; ?>" title=" "><?php echo $headerlink['title']; ?></a></li>
					<?php } ?>
				<?php } ?>
			</ul>
			</div>
			...
			<div>
				<ul>
					...
					<?php if (isset($show_account)) { ?><li class="user"><i class="fa fa-user" title=" " onclick="location='index.php?route=account/account'"></i></li><?php } ?>
					<?php echo $language; ?>
					<?php echo $currency; ?>
					<?php if (isset($show_callback)) { ?><li class="open_callback"><a onclick="$('#callback').popup('show');">Обратный звонок</a></li><?php } ?>
				</ul>
			</div>
			<?php if (isset($show_callback)) { ?><span class="open_callback dnb" onclick="$('#callback').popup('show');"><i class="fa fa-share"></i><b>Обратный звонок</b></span> <?php } ?>
		</div>
		...
		<div class="row">
			<div id="menu" class="col-xs-12 col-sm-6 col-md-12">
				<div class="open_menu" onclick="$('#column-left').toggleClass('show');"><?php if (isset($text_menu)) { ?><?php echo $text_menu; ?><?php } ?> <i class="fa fa-bars"></i></div>
				<ul>
					<?php foreach ($categories as $key => $category) { ?>
						<li>
							<a href="<?php echo $category['href']; ?>" <?php if (isset($category['active'])) { ?>class="active"<?php } ?> title=" "><?php echo $category['name']; ?></a>
